...
// Spawns the head of the snake in the center, then the rest of the snake body to its left.
void SnakeSpawn() 
{
...
void newTime()  // resets global current time
{
    seconds = time(NULL);
}

bool trophyTimer() // compares new time vs previous to check if the trophy has expired.
{
...
void snakeMove(int right, int down)
{
...
void randomTrophy() /*Trophy of rand val 1-9 is picked, it's placed on a random spot within a certain distance of the snake head so it can be acquired in time.*/
{
...
// Displays current score. Also displays win screen when score is sufficient.
int scoreTracker() 
{
    if (score == 100) 
...
// Reprograms signal handler to pause game and quit if desired.
...
int main() 
{
  ...
  while (1) 
  {
    ...
    if(trophyTimer() == true) //if timer expires, spawn new trophy.
    randomTrophy();
    ...
    */
    ...
    // Alternative version of border collision end? Remove if unwanted.
    if(snakeHitBorder) {
      attron(COLOR_PAIR(1));
